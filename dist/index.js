...
import { ListChannelsRequestSchema, PostMessageRequestSchema, ReplyToThreadRequestSchema, AddReactionRequestSchema, GetChannelHistoryRequestSchema, GetThreadRepliesRequestSchema, GetUsersRequestSchema, GetUserProfilesRequestSchema, ListChannelsResponseSchema, GetUsersResponseSchema, GetUserProfilesResponseSchema, UserProfileResponseSchema, SearchMessagesRequestSchema, SearchMessagesResponseSchema, SearchChannelsRequestSchema, SearchUsersRequestSchema, ConversationsHistoryResponseSchema, ConversationsRepliesResponseSchema, } from './schemas.js';
        ...
        return {
            tools: [
                ...
                },
                {
                    ...
                    description: 'Retrieve basic profile information of all users in the workspace',
                ...
                },
                {
                    ...
                    description: 'Search for messages with specific criteria/filters. Use this when: 1) You need to find messages from a specific user, 2) You need messages from a specific date range, 3) You need to search by keywords, 4) You want to filter by channel. This tool is optimized for targeted searches. For general channel browsing without filters, use slack_get_channel_history instead.',
                    inputSchema: zodToJsonSchema(SearchMessagesRequestSchema),
                },
                {
                    name: 'slack_search_channels',
                    description: 'Search for channels by partial name match. Use this when you need to find channels containing specific keywords in their names. Returns up to the specified limit of matching channels.',
                    inputSchema: zodToJsonSchema(SearchChannelsRequestSchema),
                },
                {
                    name: 'slack_search_users',
                    description: 'Search for users by partial name match across username, display name, and real name. Use this when you need to find users containing specific keywords in their names. Returns up to the specified limit of matching users.',
        ...
        try {
            ...
            switch (request.params.name) {
                ...
                case 'slack_get_channel_history': {
                    const args = GetChannelHistoryRequestSchema.parse(request.params.arguments);
                ...
                case 'slack_get_thread_replies': {
                    const args = GetThreadRepliesRequestSchema.parse(request.params.arguments);
                ...
                case 'slack_get_user_profiles': {
                    const args = GetUserProfilesRequestSchema.parse(request.params.arguments);
                ...
                case 'slack_search_messages': {
                    const parsedParams = SearchMessagesRequestSchema.parse(request.params.arguments);
                    ...
                    if (parsedParams.in_channel) {
                        ...
                        if (!channelInfo.ok || !channelInfo.channel?.name) {
                            throw new Error(`Failed to get channel info: ${channelInfo.error}`);
                        }
                ...
                case 'slack_search_channels': {
                    const args = SearchChannelsRequestSchema.parse(request.params.arguments);
                ...
                case 'slack_search_users': {
                    ...
                    const filteredUsers = allUsers.filter((user) => {
                        ...
                        const displayNameNormalized = user.profile?.display_name_normalized?.toLowerCase() || '';
